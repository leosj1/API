...
#     q_topterms = f'select terms from blogpost_terms where blogpost_id in ({post_ids})'
    try:
        ...
        for i in range(len(topterms_result)):
            t = topterms_result[i][2].replace('),', '----').replace('(','').replace(']','').replace('[','').replace(')','').replace('\'','').split('----')
            for elem in t:
                if elem != 'BLANK':
...
def insert_terms(tid, result, query, keyword_trend, conf):
    ...
    mycursor.execute(sql,(str(tid), str(result), str(query), str(keyword_trend)))   
    # result = mycursor.fetchall()
    mydb.commit()
...
# conf = getconf()
# print(getTerms(7, conf)[0][0])
# # end = datetime.now()
# end = time.perf_counter()
# print(end - start)

# print(keywordTrend(7,conf))
